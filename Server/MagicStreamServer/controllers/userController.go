...
func RefreshTokenHandler(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.SetCookie("access_token", newToken, 86400, "/", "localhost", false, true)          // expires in 24 hours
		c.SetCookie("refresh_token", newRefreshToken, 604800, "/", "localhost", false, true) //expires in 1 week
